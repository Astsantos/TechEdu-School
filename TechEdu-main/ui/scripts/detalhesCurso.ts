...
const pageToContentId: Record<string, number> = {
  '/Back': 2,
  '/Design': 3,
  '/Front': 1,
  '/Marketing': 4
...
async function carregarCurso() {
    try {
        const response = await fetch(`${API_URL}/api/cursos/${CURSO_ID}`, {
            method: 'GET',
        ...
        // Preenche o HTML com as informações vindas do Banco de Dados
            const titulo = document.getElementById("titulo-curso") as HTMLHeadingElement;
            console.log(titulo.innerText);
            const descricao = document.getElementById("descricao-curso") as HTMLHeadingElement;
            console.log(descricao.innerText);
            const carga_horaria = document.getElementById("cargahoraria-curso") as HTMLLIElement;
            console.log(carga_horaria.innerText);
            const matriz_curricular = document.getElementById("matriz-curso") as HTMLDivElement;
            console.log(matriz_curricular.innerText);

        const btn = document.getElementById('matricula') as HTMLButtonElement;
...
async function matricular() {
    try {
        const response = await fetch(`${API_URL}/api/cursos/${CURSO_ID}/matricular`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}` }
        });
