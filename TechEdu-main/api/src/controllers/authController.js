...
const registrar = async (req, res) => {
    ...
    try {
        const [rows] = await conexao_1.default.query('SELECT id FROM users WHERE email = ?', [email]);
        if (rows.length > 0) {
            res.status(400).json({ error: 'Este e-mail já está em uso.' });
            return;
        }
        const saltRounds = 10;
        const senhaCriptografada = await bcrypt_1.default.hash(senha, saltRounds);
        await conexao_1.default.query(`INSERT INTO users (nome, sobrenome, data_nascimento, cep, endereco, telefone, cpf, email, senha) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [nome, sobrenome, data_nascimento, cep, endereco, telefone, cpf, email, senhaCriptografada]);
...
const login = async (req, res) => {
    ...
    try {
        const [rows] = await conexao_1.default.query('SELECT * FROM users WHERE email = ?', [email]);
        if (rows.length === 0) {
